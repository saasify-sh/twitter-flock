...
class BatchJob {
  ...
  constructor(data, opts = {}) {
    ...
    if (!this._context.db) {
      console.warn(
        `Warning: no storage provided for BatchJob "${this.type}"("${this.id}")`
      )
    }
  }
  ...
  async run() {
    ...
    do {
      try {
        ...
        console.error('error running job', this.type, this.id, err)
      ...
      try {
        ...
        console.error('error saving job', this.type, this.id, err)
      }
